...
new #[Title('Student Profile')] class extends Component {
    ...
    public function student(): Student
    {
        return Student::query()->select(['id', 'admission_number', 'first_name', 'middle_name', 'last_name', 'gender', 'date_of_birth', 'photo', 'status'])
            ->with([
                'guardians' => fn ($query) => $query->select(['guardians.id', 'title', 'first_name', 'middle_name', 'last_name', 'relationship', 'phone', 'email', 'address'])->orderByPivot('is_primary', 'desc')->orderBy('last_name'),
...
<div class="grid gap-5">
    ...
    <x-app.panel title="Guardians"><div class="grid gap-4 p-4 md:grid-cols-2">@foreach($this->student->guardians as $guardian)<article class="rounded-lg border border-zinc-200 p-4 dark:border-zinc-700"><div class="flex items-start justify-between"><div><p class="font-semibold">{{ $guardian->fullName() }}</p><p class="text-sm text-zinc-500">{{ $guardian->relationship }}</p></div>@if($guardian->pivot->is_primary)<flux:badge color="blue">Primary</flux:badge>@endif</div><dl class="mt-3 grid gap-2 text-sm"><div><dt class="text-zinc-500">Phone</dt><dd>{{ $guardian->phone }}</dd></div><div><dt class="text-zinc-500">Email</dt><dd>{{ $guardian->email ?? '—' }}</dd></div><div><dt class="text-zinc-500">Address</dt><dd>{{ $guardian->address ?? '—' }}</dd></div></dl></article>@endforeach</div></x-app.panel>
