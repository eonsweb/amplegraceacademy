...
new #[Title('Add Student')] class extends Component {
    ...
    public function chooseGuardian(int $key, int $guardianId): void
    {
        ...
        $guardian = Guardian::query()->select(['id', 'first_name', 'middle_name', 'last_name', 'phone', 'email'])->findOrFail($guardianId);
        $this->guardians[$key] = ['mode' => 'existing', 'guardian_id' => $guardian->id, 'label' => $guardian->fullName(), 'title' => '', 'first_name' => '', 'middle_name' => '', 'last_name' => '', 'relationship' => '', 'phone' => '', 'email' => '', 'address' => ''];
        $this->guardianResults[$key] = [];
    }
    ...
    public function save(CreateStudent $creator): void
    {
        ...
        $rules = [
            'admissionNumber' => ['required', 'string', 'max:100', Rule::unique(Student::class, 'admission_number')],
        ...
        foreach ($this->guardians as $key => $row) {
            ...
            if (($row['mode'] ?? 'new') === 'existing') { $rules["guardians.$key.guardian_id"] = ['required', 'integer', 'distinct', Rule::exists(Guardian::class, 'id')]; continue; }
        ...
        try {
            $student = $creator->handle(
                ['admission_number' => trim($validated['admissionNumber']), 'first_name' => trim($validated['firstName']), 'middle_name' => filled($validated['middleName']) ? trim($validated['middleName']) : null, 'last_name' => trim($validated['lastName']), 'gender' => $validated['gender'], 'date_of_birth' => $validated['dateOfBirth'] ?: null, 'photo' => $storedPhoto, 'status' => $validated['status']],
        ...
        Flux::toast(variant: 'success', text: 'Student record created.');
...
<form wire:submit="save" class="grid gap-5">
    ...
    <x-app.panel title="Personal information"><div class="grid gap-4 p-4 md:grid-cols-2 xl:grid-cols-3"><flux:input wire:model="admissionNumber" label="Admission number" required /><flux:input wire:model="firstName" label="First name" required /><flux:input wire:model="middleName" label="Middle name" /><flux:input wire:model="lastName" label="Last name" required /><flux:select wire:model="gender" label="Gender" required><option value="">Select gender</option>@foreach(Gender::cases() as $item)<option value="{{ $item->value }}">{{ $item->label() }}</option>@endforeach</flux:select><flux:input wire:model="dateOfBirth" type="date" label="Date of birth" /><flux:select wire:model="status" label="Status">@foreach(StudentStatus::cases() as $item)<option value="{{ $item->value }}">{{ $item->label() }}</option>@endforeach</flux:select><flux:input wire:model="photo" type="file" label="Photo" accept="image/jpeg,image/png,image/webp" /></div></x-app.panel>
