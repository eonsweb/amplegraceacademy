...
new #[Title('Edit Student')] class extends Component {
    ...
    public function mount(Student $student): void
    {
        ...
        foreach ($student->guardians as $guardian) {
            ...
            $this->guardians[$key] = ['mode' => 'attached', 'guardian_id' => $guardian->id, 'label' => $guardian->fullName(), 'title' => $guardian->title ?? '', 'first_name' => $guardian->first_name, 'middle_name' => $guardian->middle_name ?? '', 'last_name' => $guardian->last_name, 'relationship' => $guardian->relationship, 'phone' => $guardian->phone, 'email' => $guardian->email ?? '', 'address' => $guardian->address ?? ''];
    ...
    public function save(): void
    {
        ...
        foreach ($this->guardians as $key => $row) {
            ...
            if (($row['mode'] ?? '') === 'existing') { $rules["guardians.$key.guardian_id"] = ['required', 'integer', 'distinct', Rule::exists(Guardian::class, 'id')]; continue; }
        ...
        try {
            DB::transaction(function () use ($student, $storedPhoto, $validated): void {
                ...
                foreach ($validated['guardians'] as $key => $row) {
                    if ($row['mode'] === 'new') { $guardian = Guardian::query()->create($this->guardianData($row)); $student->guardians()->attach($guardian, ['is_primary' => (int) $key === $validated['primaryGuardianKey']]); continue; }
                    $guardian = Guardian::query()->findOrFail($row['guardian_id']);
                    if ($row['mode'] === 'attached') {
                        $guardian->update($this->guardianData($row));
                        $student->guardians()->updateExistingPivot($guardian->id, ['is_primary' => (int) $key === $validated['primaryGuardianKey']]);
                    } else {
                        $student->guardians()->attach($guardian, ['is_primary' => (int) $key === $validated['primaryGuardianKey']]);
                    }
                }
            });
