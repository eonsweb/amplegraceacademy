...
<article {{ $attributes->class('flex gap-3 py-3 first:pt-0 last:pb-0') }}>
    <time class="grid size-12 shrink-0 place-items-center rounded-lg bg-brand-50 text-center text-brand-800" aria-label="{{ $month }} {{ $day }}">
    ...
    <div class="min-w-0">
        ...
        <p class="mt-0.5 text-xs text-zinc-500">{{ $date }}</p>
        <p class="mt-1 flex items-center gap-1.5 text-[11px] text-zinc-600">
